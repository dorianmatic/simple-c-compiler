...
def global_write(file, Variable):
    for loc in Variable.mem_loc:
        file.write(f"{loc}\t\tDW %D {Variable.var_value}\n")
...
def calculate_value(node, var, file, Function, index):
    # problematika s onim "abc" varijablama

    value = 0
    # ne_push[0] = False
    # op_join_rel[0]=False
    if (node.name == "<lista_izraza_pridruzivanja>"
        or node.name == "<izraz>"
        or node.name == "<lista_argumenata>"
    ):
        
        for n in node.children:
            value = calculate_value(n,var, file, Function, index)
          
    elif node.name == "<izraz_pridruzivanja>":

        if len(node.children) > 1:
            # op_join_rel[0]=True
            value = calculate_value(node.children[2], var, file, Function , index)
            pop(file, "R0", Function)
            op_join[0] = True
            value = calculate_value(node.children[0], var, file, Function, index)
            op_join[0] = False
        elif node.parent.name == "<lista_izraza_pridruzivanja>":
      
                value = calculate_value(node.children[0], var, file, Function, index)
              
                mem_loc = var.mem_loc[var.id]
                if var.list == True and var.string_var == False:
                    var.id += 1
                    pop(file, "R0", Function)
                    file.write(f"\t\tSTORE R0, ({mem_loc})\n")
        ...
        if len(node.children) == 2:
            ...
            if node.children[1].name == "L_ZAGRADA":
                ...
                value = calculate_value(
                    node.children[0].children[0], var, file, Function, index
                )
            ...
            if children_names == ["IDN"]:
                ...
                if index != 2:
                    if int(value) > 524287 or int(value) < -524288:
                        new_var = Variable()
                        new_var.mem_loc = [f"B_{ind[0]}"]
                        new_var.var_value = value
                        value = new_var
                        scope_mem_loc[level[0]].append(new_var)
                        file.write(f"\t\tLOAD R0, ({new_var.mem_loc[0]})\n")
                        push(file, "R0", Function)
                    else:
        ...
        if children_names == ["IDN"]:
            ...
            for char in value:
                ...
                push(file, "R0", Function)
                pop(file, "R0", Function)
                file.write(f"\t\tSTORE R0, ({var.mem_loc[var.id]})\n")
            
                var.id += 1
        else:
            value = calculate_value(node.children[1], var, file, Function, index)
        ...
        if len(node.children) > 2:
            if node.name == "<log_i_izraz>":
                ...
                value = calculate_value(
                    node.children[0], var, file, Function, index
                )
            ...
            if node.name == "<unarni_izraz>":
                value = calculate_value(
                    node.children[1], var, file, Function, index
                )
                if node.children[0].name == "OP_INC" or node.children[0].name == "OP_DEC":
                    name = value.upper()
                    mem_loc = find_mem_loc_name(file, name, Function)
...
def block_analysis(file, node, Function, var_list, con_br):
    ...
    if (
        ...
        if node.children[0].name == "<slozena_naredba>":
            ...
            con_br = (f"NEXT_{ind[0]}",f"NEXT_{ind[0]}")
            temp_node = node.children[0]
            value = calculate_value(temp_node.children[2],var = None,file=file,Function=Function,index=-1)
            if isinstance(value,Variable):
                var_list.append(value)
            pop(file,"R0",Function)
            file.write("\t\tCMP R0, 0\n")
            ...
            if temp_node.children[0].name == "KR_WHILE":
                ...
                if len(izraz1.children)>1:
                    value = calculate_value(izraz1.children[0],var=None, file=file, Function=Function, index = 4)
                    if isinstance(value,Variable):
                        var_list.append(value)
                file.write(f"{con_br[0]}\n")
                if len(izraz2.children)>1:
                    value = calculate_value(izraz2.children[0],var=None, file=file, Function=Function, index = -1)
                    if isinstance(value,Variable):
                        var_list.append(value)
                    pop(file, "R0", Function)
                    file.write("\t\tCMP R0, 0\n")
                ...
                if len(temp_node.children) > 6:
                    ...
                    if Function.stog > 0:
                        for i in range(Function.stog):
                            pop(file, "R0", Function)
                    value = calculate_value(izraz3.children[0],var=None, file=file, Function=Function, index = -1)
                    if isinstance(value,Variable):
                        var_list.append(value)
                    pop(file,"R0",Function)
                else:
